...
| 键盘背光             | 正常                 | 已经完美驱动，可6级调节，调节可以在设置→键盘，取消暗光自动可调出自感应模式。快捷键：`Alt（目前为CMD键） + 亮度+ 或 亮度- `                                                                |           
| 功能快捷键           | 正常                 | 屏幕亮度、键盘亮度调节和声音调节正常使用，完整功能需要尝试重新映射（正在修复）                                                        |
...
| 内置外放             | 禁用               | 声卡驱动`ChultraSOF.kext`测试阶段，可使用蓝牙音响或耳机平替                                                               |
